...
app.post('/api/pages/:id/attachments', authenticate, requirePermission('pages.edit'), writeLimiter, (req, res) => {
  if (!pool) return res.status(503).json({ error: 'Database not connected' });
...
app.get('/api/pages/:id/attachments', authenticate, requirePermission('pages.read'), async (req, res) => {
  if (!pool) return res.status(503).json({ error: 'Database not connected' });
  const pageId = parseInt(req.params.id);
  if (isNaN(pageId)) return res.status(400).json({ error: 'Invalid page ID' });
  try {
...
// Download attachment
app.get('/api/attachments/:id/download', authenticate, requirePermission('pages.read'), async (req, res) => {
  if (!pool) return res.status(503).json({ error: 'Database not connected' });
  const id = parseInt(req.params.id);
...
// Delete attachment
app.delete('/api/attachments/:id', authenticate, requirePermission('pages.edit'), writeLimiter, async (req, res) => {
  if (!pool) return res.status(503).json({ error: 'Database not connected' });
  const id = parseInt(req.params.id);
  if (isNaN(id)) return res.status(400).json({ error: 'Invalid attachment ID' });
  try {
